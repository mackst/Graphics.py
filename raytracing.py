...
class Sphere(object):
    ...
    def __init__(self, pos, radius, diffuse, specular):
        self.pos = pos
        self.radius = radius
    ...
    def toArray(self):
        return np.array([self.pos[0], self.pos[1], self.pos[2], self.radius,
                         self.diffuse[0], self.diffuse[1], self.diffuse[2], self.specular,
                         self.id,
                         -1., -1., -1.], np.float32)
...
class Plane(object):
    def __init__(self, normal, distance, diffuse, specular):
        self.normal = normal
    ...
    def toArray(self):
        return np.array([self.normal[0], self.normal[1], self.normal[2], self.distance,
                         self.diffuse[0], self.diffuse[1], self.diffuse[2], self.specular,
                         self.id,
                         -1., -1., -1.], np.float32)
...
class Camera(object):
    ...
    def __init__(self, pos, lookat, fov):
        self.pos = pos
        self.lookat = lookat
        self.fov = fov
...
class ComputeApplication(object):
    ...
    def __init__(self):
        ...
        self.__spheresBuffer = None
        self.__spheresBufferMemory = None

        self.__planesBuffer = None
        self.__planesBufferMemory = None

        # size of `buffer` in bytes.
        self.__bufferSize = 0
        self.__spheresBufferSize = 0
        self.__plansBufferSize = 0
    ...
    def __del__(self):
        ...
        if self.__spheresBufferMemory:
            vkFreeMemory(self.__device, self.__spheresBufferMemory, None)
        if self.__spheresBuffer:
            vkDestroyBuffer(self.__device, self.__spheresBuffer, None)

        if self.__planesBufferMemory:
            vkFreeMemory(self.__device, self.__planesBufferMemory, None)
        if self.__planesBuffer:
            vkDestroyBuffer(self.__device, self.__planesBuffer, None)
    ...
    def createBuffer(self):
        ...
        uboArray = np.array((
            ...
            0, 0, 0, 0,
            # camera
            0, 0, 4.0, #0.0, 0.5, 0.0, 10.0
        ), np.float32)
        self.__uboBufferSize = uboArray.nbytes

        rot = math.radians((random.random() * 2 - 1) * 360)
        # rot = math.radians(random.random() * -1 * 360)
        # rot = math.radians(360)
        # rot = (random.random() * 2 - 1) * 360
        uboArray[0] = math.sin(rot) * math.cos(rot) * 2.0
        uboArray[1] = math.sin(rot) * 2.0
        uboArray[2] = math.cos(rot) * 2.0
        ...
        # spheres buffer
        spheres = (
            Sphere(np.array((1.75, -0.5, 0.0)), 1.0, np.array((0.0, 1.0, 0.0)), 32.0).toArray(),
            Sphere(np.array((0.0, 1.0, -0.5)), 1.0, np.array((0.65, 0.77, 0.97)), 32.0).toArray(),
            Sphere(np.array((-1.75, -0.75, -0.5)), 1.25, np.array((0.9, 0.76, 0.46)), 32.0).toArray(),
        )
        spheresData = np.concatenate(spheres)
        spheresData = spheresData.astype(np.float32)
        self.__spheresBufferSize = spheresData.nbytes

        bufferCreateInfo = VkBufferCreateInfo(
            size=self.__spheresBufferSize,  # buffer size in bytes.
            usage=VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,  # buffer is used as a storage buffer.
            sharingMode=VK_SHARING_MODE_EXCLUSIVE  # buffer is exclusive to a single queue family at a time.
        )

        self.__spheresBuffer = vkCreateBuffer(self.__device, bufferCreateInfo, None)

        memoryRequirements = vkGetBufferMemoryRequirements(self.__device, self.__spheresBuffer)
        index = self.findMemoryType(memoryRequirements.memoryTypeBits,
                                    VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
        allocateInfo = VkMemoryAllocateInfo(
            allocationSize=memoryRequirements.size,  # specify required memory.
        ...
        self.__spheresBufferMemory = vkAllocateMemory(self.__device, allocateInfo, None)

        data = vkMapMemory(self.__device, self.__spheresBufferMemory, 0, self.__spheresBufferSize, 0)
        dptr = ffi.cast('float *', spheresData.ctypes.data)
        ffi.memmove(data, dptr, self.__spheresBufferSize)
        # ffi.memmove(data, spheres, self.__spheresBufferSize)
        vkUnmapMemory(self.__device, self.__spheresBufferMemory)

        vkBindBufferMemory(self.__device, self.__spheresBuffer, self.__spheresBufferMemory, 0)

        # plane buffer
        planes = (
            Plane(np.array((0.0, 1.0, 0.0)), 4.0, np.array((1.0, 1.0, 1.0)), 32.0).toArray(),
            Plane(np.array((0.0, -1.0, 0.0)), 4.0, np.array((1.0, 1.0, 1.0)), 32.0).toArray(),
            Plane(np.array((0.0, 0.0, 1.0)), 4.0, np.array((1.0, 1.0, 1.0)), 32.0).toArray(),
            Plane(np.array((0.0, 0.0, -1.0)), 4.0, np.array((0.0, 0.0, 0.0)), 32.0).toArray(),
            Plane(np.array((-1.0, 0.0, 0.0)), 4.0, np.array((1.0, 0.0, 0.0)), 32.0).toArray(),
            Plane(np.array((1.0, 0.0, 0.0)), 4.0, np.array((0.0, 1.0, 0.0)), 32.0).toArray(),
        )
        planesData = np.concatenate(planes)
        planesData = planesData.astype(np.float32)
        self.__plansBufferSize = planesData.nbytes

        bufferCreateInfo = VkBufferCreateInfo(
            size=self.__plansBufferSize,  # buffer size in bytes.
            usage=VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,  # buffer is used as a storage buffer.
            sharingMode=VK_SHARING_MODE_EXCLUSIVE  # buffer is exclusive to a single queue family at a time.
        )

        self.__planesBuffer = vkCreateBuffer(self.__device, bufferCreateInfo, None)

        memoryRequirements = vkGetBufferMemoryRequirements(self.__device, self.__planesBuffer)
        index = self.findMemoryType(memoryRequirements.memoryTypeBits,
                                    VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
        allocateInfo = VkMemoryAllocateInfo(
            allocationSize=memoryRequirements.size,  # specify required memory.
        ...
        self.__planesBufferMemory = vkAllocateMemory(self.__device, allocateInfo, None)

        data = vkMapMemory(self.__device, self.__planesBufferMemory, 0, self.__plansBufferSize, 0)
        dptr = ffi.cast('float *', planesData.ctypes.data)
        ffi.memmove(data, dptr, self.__plansBufferSize)
        # ffi.memmove(data, planes, self.__plansBufferSize)
        vkUnmapMemory(self.__device, self.__planesBufferMemory)

        vkBindBufferMemory(self.__device, self.__planesBuffer, self.__planesBufferMemory, 0)
    ...
    def createDescriptorSet(self):
        ...
        spheresBufferInfo = VkDescriptorBufferInfo(
            buffer=self.__spheresBuffer,
            offset=0,
            range=self.__spheresBufferSize
        )

        planeBufferInfo = VkDescriptorBufferInfo(
            buffer=self.__planesBuffer,
            offset=0,
            range=self.__plansBufferSize
        )
        ...
        writeDescriptorSets = [
            ...
            VkWriteDescriptorSet(
                ...
                descriptorCount=1,
                descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                pBufferInfo=spheresBufferInfo
            ),

            VkWriteDescriptorSet(
                dstSet=self.__descriptorSet,
                dstBinding=3,  # write to the first, and only binding.
                descriptorCount=1,
                descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                pBufferInfo=planeBufferInfo
            )
        ]
    ...
    def createComputePipeline(self):
        ...
        with open('raytracing.spv', 'rb') as comp:
            code = comp.read()
